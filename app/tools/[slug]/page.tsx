...
export async function generateMetadata({
  ...
  if (doc) {
    return {
      title: `${doc.title} - 80 Days to Stay Tools`,
      description: doc.description || `${doc.title} — tool by 80 Days to Stay`,
    }
  }
  ...
  try {
    ...
    if (rows.length > 0) {
      return {
        title: `${rows[0].name} - 80 Days to Stay Tools`,
        description: rows[0].description || `${rows[0].name} — tool by 80 Days to Stay`,
      }
    }
  } catch {}

  return { title: 'Tool - 80 Days to Stay' }
}
